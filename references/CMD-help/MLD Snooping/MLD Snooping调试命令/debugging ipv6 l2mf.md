...
**[debugging ipv6 l2mf **[{ **all** \| **error** \| **event** \| **group** \| **msg** }]]

**[undo debugging ipv6 l2mf **[{ **all** \| **error** \| **event** \| **group** \| **msg** }]]
...
**[all**]：表示IPv6 L2MF（Layer-2 Multicast Forwarding，二层组播转发）所有调试信息开关。

**[error**]：表示IPv6 L2MF错误调试信息开关。

**[event**]：表示IPv6 L2MF事件调试信息开关。

**[group**]：表示IPv6 L2MF组播组调试信息开关。

**[msg**]：表示IPv6 L2MF消息调试信息开关。

【描述】

**[debugging ipv6 l2mf**]命令用来打开IPv6 L2MF调试信息开关。**undo debugging ipv6 l2mf**命令用来关闭IPv6 L2MF调试信息开关。

缺省情况下，IPv6 L2MF调试信息开关处于关闭状态。

表1-1 debugging ipv6 l2mf error命令输出信息描述表
...
表1-2 debugging ipv6 l2mf event命令输出信息描述表

字段

描述

Port is exisit when add port for entry (0::0) in VLAN *vlan*

在VLAN *vlan*中为表项（0::0）添加端口时，该端口已存在

Add port for entry (0::0) inVLAN *vlan*

在VLAN *vlan*中为表项（0::0）添加端口

Delete port for entry (0::0) in VLAN *vlan*

在VLAN *vlan*中为表项（0::0）删除端口

Add slot *slot* for entry (0::0) in VLAN*vlan*

在VLAN *vlan*中为表项（0::0）添加板*slot*

表1-3 debugging ipv6 l2mf group命令输出信息描述表
...
表1-4 debugging ipv6 l2mf msg命令输出信息描述表
...
\<Sysname\> debugging ipv6 l2mf event

\*Jun  4 12:55:19:912 2012 Sysname L2MF/7/EVENT: -MDC=1; Delete port for entry (0::0) in VLAN 2. (A171255)

*// 在VLAN 2中为表项（0::0）删除端口*

\# 打开IPv6 L2MF组播组调试信息开关。

\<Sysname\> debugging ipv6 l2mf group

\*Jun  4 12:50:41:191 2012 Sysname L2MF/7/GROUP: -MDC=1; Add MAC entry 3333-0000-0001 in VLAN 1. (A151783)

*// 在VLAN 1中添加MAC表项3333-0000-0001*

\*Jun  4 12:50:41:191 2012 Sysname L2MF/7/GROUP: -MDC=1; Add entry (::, ff1e::1) in VLAN 1. (A151071)

*// 在VLAN 1中添加表项（::，FF1E::1）*

\# 打开IPv6 L2MF消息调试信息开关。

\<Sysname\> debugging ipv6 l2mf msg

\*Jun  4 12:48:53:840 2012 Sysname L2MF/7/MESSAGE: -MDC=1; Flush add entry Mac message (3333-0000-0001) to driver in VLAN 1. (A141939)

*// 在VLAN 1中将MAC表项3333-0000-0001的信息下刷驱动*

\*Jun  4 12:48:53:840 2012 Sysname L2MF/7/MESSAGE: -MDC=1; Save message to Kernel. (A131801)

*// 保存信息到内核*

\*Jun  4 12:48:53:840 2012 Sysname L2MF/7/MESSAGE: -MDC=1; Process add entry message for MAC entry 3333-0000-0001  (A18873)

*// 处理添加MAC表项3333-0000-0001的消息*

\*Jun  4 12:48:53:840 2012 Sysname L2MF/7/MESSAGE: -MDC=1; Flush add entry message (::, ff1e::1) to driver in VLAN 1. (A141815)

*// 在VLAN 1中将表项（::，FF1E::1）的信息下刷驱动*

\*Jun  4 12:48:53:840 2012 Sysname L2MF/7/MESSAGE: -MDC=1; Process add entry message for entry (::, ff1e::1). (A18815)

*// 处理添加IP表项（::，FF1E::1）的消息*

**MLD Snooping \-- MLD Snooping调试命令 \-- debugging mld-snooping**

------------------------------------------------------------------------

【命令】

**[debugging mld-snooping **[{ **all** \| **entry** \| **error** \| **event** \| **fsm** \| **group** \| **packet** [ **vlan** *vlan-id* [ **port** *interface-type interface-number* ] \| **vsi** *vsi-name* ] \| **sync** \| **timer** }]]

**[undo debugging mld-snooping **[{ **all** \| **entry** \| **error** \| **event** \| **fsm** \| **group** \| **packet** \| **sync** \| **timer** }]]
...
**[all**]：表示MLD Snooping所有调试信息开关。

**[entry**]：表示MLD Snooping表项调试信息开关。

**[error**]：表示MLD Snooping错误调试信息开关。

**[event**]：表示MLD Snooping事件调试信息开关。

**[fsm**]：表示MLD Snooping状态机调试信息开关。

**[group**]：表示MLD Snooping组播组调试信息开关。

**[packet**]：表示MLD Snooping报文调试信息开关。
...
**[sync**]：表示MLD Snooping板间消息同步调试信息开关。

**[timer**]：表示MLD Snooping定时器调试信息开关。

【描述】

**[debugging mld-snooping**]命令用来打开MLD Snooping调试信息开关。**undo debugging mld-snooping**命令用来关闭MLD Snooping调试信息开关。

缺省情况下，MLD Snooping调试信息开关处于关闭状态。

表1-5 debugging mld-snooping entry命令输出信息描述表
...
表1-6 debugging mld-snooping error命令输出信息描述表
...
Wrong MLDv1 report packet which receive from port *port* on *vlan*

在VLAN *vlan*的端口*port*上收到错误的MLDv1加入报文
...
表1-7 debugging mld-snooping event命令输出信息描述表

字段

描述

Successfully enable/disable MLD snooping on *vlan*

在VLAN *vlan*上使能/关闭MLD Snooping成功

Received get level2 multicast IP/mac group message

收到获取二层组播IP/MAC组信息

Received MLD snooping debug message

收到MLD Snooping调试信息

Received MLD snooping group message

收到MLD Snooping组信息

Received MLD snooping router port message

收到MLD Snooping路由器端口信息
...
Global MLD snooping is enabled

MLD Snooping已全局使能

MLD snooping is disabling globally

MLD Snooping正在全局关闭

MLD snooping is enabled/disabled on *vlan*

MLD Snooping在VLAN *vlan*上已使能/关闭
...
表1-8 debugging mld-snooping fsm命令输出信息描述表
...
表1-9 debugging mld-snooping group命令输出信息描述表
...
表1-10 debugging mld-snooping packet命令输出信息描述表

字段

描述

Succeed in forwarding MLD packet to port *port* on *vlan*

成功发送MLD报文到VLAN *vlan*上的端口*port*
...
Receive MLDv*n* version general query packet from port *port* on *vlan*

从VLAN *vlan*的端口*port*上获取版本*n*的MLD通用查询报文

Receive MLDv1 group specific query packet from port *port* on *vlan*

从VLAN *vlan*的端口*port*上获取MLDv1特定组查询报文

Receive MLDv2 group specific query packet from port *port* on *vlan*

从VLAN *vlan*的端口*port*上获取MLDv2特定组查询报文

Receive MLDv1 report packet from port *port* on *vlan*

从VLAN *vlan*的端口*port*上获取MLDv1报告报文

Receive MLD done packet from port *port* on *vlan*

从VLAN *vlan*的端口*port*上获取MLD离开报文

The version of MLD packet that receive from port *port* on *vlan* is higher than the version on *vlan*
...
Holdtime option is zero in PIMv2 Hello packet which received from port *port* on *vlan*

从VLAN *vlan*的端口*port*上获取到PIMv2 Hello报文holdtime为0时，不维护路由器端口

Deal with the MLD/PIMv2 packet which receive from port *port* on *vlan*

处理从VLAN *vlan*的端口*port*上获取到的MLD/PIMv2报文
...
转发从VLAN *vlan*的端口*port*上获取的（*source*，*group*）的特定源组查询报文

Forward the MLD report packet on router port which destination IP address is *group* and source IP address is *source* that receive from port *port* on *vlan*

从路由器端口上转发从VLAN *vlan*的端口*port*上获取的（*source*，*group*）的MLD报告报文

The version of MLD packet is lower than version on *vlan*, the main slot broadcast it on *vlan*

报文版本低于端口版本，主板在VLAN *vlan*内广播

The MLD packet which receive from port *port* on *vlan* on Main slot, forward it locally

从主板VLAN *vlan*的端口*port*上获取报文，本地转发

The MLD packet version which receive from MLD is higher than the version on *vlan*, broadcast it on *vlan*
...
The MLD packet which receive from MLD on *vlan* on main slot, forward it locally

从主板VLAN *vlan*上获取的报文，本地转发

Forward the MLD packet locally

本地转发MLD报文

Send the MLD packet up to IP
...
Host send MLDv2(*mode*) packet to *port* with *group* and 0 source(s) on *vlan*.

VLAN *vlan*内的模拟主机向端口*port*发送模式为*mode*的MLDv2报告报文

表1-11 debugging mld-snooping sync命令输出信息描述表
...
Main slot forward the MLD packet to other IO slot

主板发送MLD报文至接口板

The MLD packet which receive from port *port* on *vlan* on IO slot, send it to the main slot

发送来自于VLAN *vlan*上端口*port*的接口板上的消息至主板

Send the MLD packet to IO slots

发送MLD报文至接口板

Receive MLD packet from another slot

从其它板收到MLD报文

Synchronize vlan or vsi MLD snooping disable message to other slots

同步VLAN的MLD Snooping去使能信息到其它板
...
Successfully recover global MLD snooping disabled and synchronize message to other slots
...
Successfully recover MLD snooping disable on *vlan* and send message to other slots synchronize configuration

恢复VLAN *vlan*上去使能并同步到其它板

Successfully recover MLD snooping drop-unknown disable on *vlan* and send message to other slots synchronize configuration

恢复VLAN *vlan*上未知组播丢弃并同步到其它板

Successfully recover MLD snooping Version on *vlan* and send message to other slots synchronize configuration

恢复VLAN *vlan*上版本并同步到其它板

Successfully recover MLD snooping router-aging-time on *vlan* and send message to other slots synchronize configuration

恢复VLAN *vlan*上路由端口定时器并同步到其它板

Successfully recover MLD snooping max-response-time on *vlan* and send message to other slots synchronize configuration
...
Synchronize enable or disable MLD snooping globally message to other slots
...
表1-12 debugging mld-snooping timer命令输出信息描述表
...
Successfully create MLDv1 query present timer on *vlan*, *n* seconds.

在VLAN *vlan*内将MLDv1查询存在定时器创建为*n*秒

Successfully resize MLDv1 query present timer on *vlan*, *n* seconds.

在VLAN *vlan*内将MLDv1查询存在定时器调整为*n*秒

The MLDv1 query present times out on *vlan*.

VLAN *vlan*内的MLDv1查询存在定时器超时
...
\# 在设备上使能MLD Snooping，打开MLD Snooping接口驱动调试信息开关。

\<Sysname\> debugging mld-snooping entry

\*Sep 15 11:43:28:565 2011 Sysname MCS6/7/ENTRY: -MDC=1; Delete MAC entry 3333-0001-0101 from driver. (G156098)

*// 在VLAN 2内从驱动中删除二层MAC表项*

\# 在设备上使能MLD Snooping，打开MLD Snooping事件调试信息开关。

\<Sysname\> debugging mld-snooping event

\*Sep 15 11:46:06:924 2011 Sysname MCS6/7/EVENT: -MDC=1; Successfully enable MLD snooping on VLAN 4. (G174304)

*// 在VLAN 4上使能MLD Snooping*

\# 在设备上使能MLD Snooping，打开MLD Snooping组播组调试信息开关。

\<Sysname\> debugging mld-snooping group

\*Sep 15 11:47:41:455 2011 Sysname MCS6/7/GROUP: -MDC=1; Create Dynamic group (1::1, FF0E::225:0:0:1) on VLAN 2, add host port GE1/0/1. (G091840)

*// 在VLAN 2内创建动态表项（1::1，FF0E::225:0:0:1），添加主机端口GigabitEthernet1/0/1*

\# 在设备上使能MLD Snooping，打开MLD Snooping报文调试信息开关。

\<Sysname\> debugging mld-snooping packet

\*Sep 15 14:43:41:062 2011 Sysname MCS6/7/PACKET: -MDC=1; Receive MLDv1 report packet from port GE1/0/1 on VLAN 2. (G172586)

*// 在VLAN 2内的端口GigabitEthernet1/0/1上收到MLDv1成员关系报告报文*

\# 在设备上使能MLD Snooping，打开MLD Snooping板间同步调试信息开关。

\<Sysname\> debugging mld-snooping sync

\*Sep 15 13:40:04:692 2011 Sysname MCS6/7/SYNC: -MDC=1; synchronize configure debug message to other board. (G1710245)

*// 通知其它板打开调试信息*

\# 在设备上使能MLD Snooping，打开MLD Snooping定时器调试信息开关。

\<Sysname\> debugging mld-snooping timer

\*Sep 15 14:43:41:062 2011 Sysname MCS6/7/TIMER: -MDC=1; Create host port timer, 260 seconds, for port: GE1/0/1, dynamic group:(::, FF1E::1) on VLAN 1. (G09264)

*// 创建成员端口，并将其老化时间设置为260秒*

\*Sep 15 14:48:01:292 2011 Sysname MCS6/7/TIMER: -MDC=1; The host port GE1/0/1 times out for dynamic group (::, FF1E::1) on VLAN 1. (G092579)

*// 端口定时器超时*

\# 在设备上使能MLD Snooping，打开MLD Snooping状态机调试信息开关。

\<Sysname\> debugging mld-snooping fsm

\*Sep 15 14:43:41:062 2011 Sysname MCS6/7/FSM: -MDC=1; Notified add host self slot 0 of (::,FF1E::1) to other on VLAN 2. (G061062)
