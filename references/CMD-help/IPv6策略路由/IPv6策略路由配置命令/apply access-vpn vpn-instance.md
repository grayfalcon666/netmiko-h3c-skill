...
![说明](IPv6策略路由命令.files/image001.png)
...
Sysnameipv6 policy-based-route policy1 permit node 10
...
*[ipv6-address*]：缺省下一跳的IPv6地址。不指定**vpn-instance**或**inbound-vpn**参数，表示指定的是公网下一跳。
...
配置**undo**命令时，如果指定了缺省下一跳IPv6地址，将取消已配置的该缺省下一跳；如果没有指定缺省下一跳IPv6地址，将取消已配置的所有缺省下一跳。
...
*[ipv6-address*]：下一跳IPv6地址。不指定**vpn-instance**或**inbound-vpn**参数，表示指定的是公网下一跳。
...
配置**undo**命令时，如果指定了下一跳IPv6地址，将取消已配置的该下一跳；如果没有指定下一跳IPv6地址，将取消已配置的所有下一跳。
...
配置**undo**命令时，如果指定了接口，将取消已配置的该出接口；如果没有指定接口，将取消已配置的所有出接口。

【举例】

\# 对已经匹配的IPv6报文指定出接口为GigabitEthernet1/0/1。
...
![说明](IPv6策略路由命令.files/image002.png)

本命令的支持情况与设备的型号有关，请以设备的实际情况为准。

**[apply precedence**]命令用来设置IPv6报文的优先级。
...
*[value*]：IPv6报文的优先级值，IPv6报文共有8（0～7）个优先级，每个数值对应一个优先级类型。在输入参数的时候可以输入数值，也可以输入优先级类型。对应关系如**[错误！未找到引用源。]**(#_Ref329186298)所示。

表1-1 IPv6优先级值与优先级类型对应表
...
\# 设置IPv6报文的优先级为5（critical）。

\<Sysname\> system-view

Sysnameipv6 policy-based-route aa permit node 11
...
**[display** **ipv6 policy-based-route**]命令用来显示已经配置的IPv6策略。
...
**[policy*** policy-name*]：显示指定的IPv6策略。*policy-name*表示策略名，唯一标识一个IPv6策略，为1～19个字符的字符串，区分大小写。

【使用指导】

如果不指定策略名，将显示所有已经配置的IPv6策略；如果指定策略名，将显示指定的IPv6策略。
...
表1-2 display ipv6 policy-based-route命令显示信息描述表
...
**[display** **ipv6 policy-based-route interface**]命令用来显示接口下IPv6转发策略路由的配置信息和统计信息。
...
**[display ipv6 policy-based-route interface ***interface-type interface-number* [ **slot** *slot-number* [ **cpu** *cpu-number*  ]]]

分布式设备－IRF模式：

**[display ipv6 policy-based-route interface ***interface-type interface-number* **chassis** *chassis-number* **slot** *slot-number*  **cpu** *cpu-number*  ]
...
**[slot*** slot-number*]：显示指定单板上IPv6转发策略路由的配置信息和统计信息。*slot-number*表示单板所在的槽位号。（分布式设备－独立运行模式）

**[slot*** slot-number*]：显示指定成员设备上IPv6转发策略路由的配置信息和统计信息。*slot-number*表示设备在IRF中的成员编号。（集中式IRF设备）（不支持IRF3的设备）

**[slot ***slot-number*]：显示指定成员设备/PEX上的IPv6转发策略路由的配置信息和统计信息，*slot-number*表示设备在IRF中的成员编号或者PEX的虚拟槽位号。（集中式IRF设备）（支持IRF3的设备）

**[chassis** *chassis-number* **slot** *slot-number*]：显示指定成员设备上指定单板上IPv6转发策略路由的配置信息和统计信息。*chassis-number*表示设备在IRF中的成员编号，*slot-number*表示单板所在的槽位号。（分布式设备－IRF模式）（不支持IRF3的设备）

**[chassis** *chassis-number* **slot** *slot-number*]：显示指定单板上的IPv6转发策略路由的配置信息和统计信息，*chassis-number*表示设备在IRF中的成员编号或者PEX对应的虚拟框号，*slot-number*表示单板或PEX所在的槽位号。（分布式设备－IRF模式）（支持IRF3的设备）

**[cpu** *cpu-number*]：显示指定CPU上IPv6转发策略路由的配置信息和统计信息，*cpu-number*表示CPU的编号。只有指定的**slot**支持多CPU时，才能配置该参数。本参数的支持情况与设备的型号有关，请以设备的实际情况为准。（分布式设备－独立运行模式/集中式IRF设备/分布式设备－IRF模式）

【举例】

\# 显示接口GigabitEthernet1/0/1下IPv6转发策略路由的配置信息和统计信息。
...
表1-3 display ipv6 policy-based-route interface命令显示信息描述表
...
接口GigabitEthernet1/0/1下IPv6转发策略路由的配置信息和统计信息（failed表示策略下发驱动失败，此时所有节点都下发失败，不再显示节点一级的失败提示）
...
·**reset ip****v6 policy-based-route statistics**

**IPv6策略路由 \-- IPv6策略路由配置命令 \-- display ipv6 policy-based-route local**

------------------------------------------------------------------------

**[display ipv6 policy-based-route local**]命令用来显示IPv6本地策略路由的配置信息和统计信息。
...
**[display ipv6 policy-based-route local** [ **slot** *slot-number* [ **cpu** *cpu-number* ]]]

分布式设备－IRF模式：

**[display ipv6 policy-based-route local ** **chassis** *chassis-number* **slot** *slot-number*  **cpu** *cpu-number*  ]
...
**[slot*** slot-number*]：显示指定单板上IPv6本地策略路由的配置信息和统计信息。*slot-number*表示单板所在的槽位号。（分布式设备－独立运行模式）

**[slot*** slot-number*]：显示指定成员设备上IPv6本地策略路由的配置信息和统计信息。*slot-number*表示设备在IRF中的成员编号。（集中式IRF设备）（不支持IRF3的设备）

**[slot ***slot-number*]：显示指定成员设备/PEX上的IPv6本地策略路由的配置信息和统计信息，*slot-number*表示设备在IRF中的成员编号或者PEX的虚拟槽位号。（集中式IRF设备）（支持IRF3的设备）

**[chassis** *chassis-number* **slot** *slot-number*]：显示指定成员设备上指定单板上IPv6本地策略路由的配置信息和统计信息。*chassis-number*表示设备在IRF中的成员编号，*slot-number*表示单板所在的槽位号。（分布式设备－IRF模式）（不支持IRF3的设备）

**[chassis** *chassis-number* **slot** *slot-number*]：显示指定单板上的IPv6本地策略路由的配置信息和统计信息，*chassis-number*表示设备在IRF中的成员编号或者PEX对应的虚拟框号，*slot-number*表示单板或PEX所在的槽位号。（分布式设备－IRF模式）（支持IRF3的设备）

**[cpu** *cpu-number*]：显示指定CPU上IPv6本地策略路由的配置信息和统计信息，*cpu-number*表示CPU的编号。只有指定的**slot**支持多CPU时，才能配置该参数。本参数的支持情况与设备的型号有关，请以设备的实际情况为准。（分布式设备－独立运行模式/集中式IRF设备/分布式设备－IRF模式）
...
表1-4 display ipv6 policy-based-route local命令显示信息描述表
...
**[display** **ipv6 policy-based-route setup**]命令用来显示已经应用的IPv6策略路由信息。
...
表1-5 display ipv6 policy-based-route setup命令显示信息描述表
...
*[acl6-number*]：访问控制列表号，取值范围为2000～3999。其中：

·基本IPv6 ACL，*acl6-number*取值范围为2000～2999；

·高级IPv6 ACL，*acl6-number*取值范围为3000～3999。

**[name*** acl6-name*]：指定IPv6 ACL的名称。*acl6-name*表示IPv6 ACL的名称，为1～63个字符的字符串，不区分大小写，必须以英文字母a～z或A～Z开头。为避免混淆，IPv6 ACL的名称不允许使用英文单词all。
...
**[if-match packet-length**]命令用来设置IPv6报文长度匹配规则。

**[undo if-match packet-length**]命令用来删除IPv6报文长度匹配规则的设置。
...
*[min-len*]：最短IPv6报文长度，取值范围为1～65535，单位为字节。

*[max-len*]：最长IPv6报文长度，取值范围为1～65535，单位为字节。*max-len*应该不小于*min-len*。

【使用指导】

长度匹配含边界值，如指定min-len为100，max-len为200，报文长度为100与200的报文都是匹配报文。
...
**[ipv6 local policy-based-route**]命令用来对本地报文应用IPv6策略。

**[undo ipv6 local policy-based-route**]命令用来删除对本地报文应用IPv6策略的设置。
...
*[policy-name*]：策略名，唯一标识一个IPv6策略，为1～19个字符的字符串，区分大小写。该IPv6策略必须已经存在。

【使用指导】

对本地报文只能应用一个IPv6策略。应用新的IPv6策略前必须删除本地原来已经应用的IPv6策略。

对本地报文应用的IPv6策略将对本地产生的所有IPv6报文进行匹配。若无特殊需求，建议用户不要配置本地策略路由。
...
·**display ip****v6 policy-based-route setup**

·**ipv6 poli****cy-based-route (System view)**

**IPv6策略路由 \-- IPv6策略路由配置命令 \-- ipv6 policy-based-route (interface view)**

------------------------------------------------------------------------

**[ipv6 policy-based-route**]命令用来对接口转发的报文应用IPv6策略。

**[undo ipv6 policy-based-route**]命令用来取消对接口转发的报文应用IPv6策略。
...
\# 对接口GigabitEthernet1/0/1转发的报文应用IPv6策略aaa。
...
\# 对接口Vlan-interface2转发的报文应用IPv6策略aaa。
...
**[ipv6 policy-based-route**]命令用来创建IPv6策略节点，并进入IPv6策略节点视图。如果指定的IPv6策略节点已创建，则该命令直接用来进入该IPv6策略节点的视图。

**[undo ipv6 policy-based-route**]命令用来删除已创建的IPv6策略或IPv6策略节点。
...
*[policy-name*]：策略名，唯一标识一个IPv6策略，为1～19个字符的字符串，区分大小写。

**[deny**]：指定节点的匹配模式为拒绝模式。

**[permit**]：指定节点的匹配模式为允许模式。缺省匹配模式为**permit**。

**[node*** node-number*]：IPv6策略节点。节点编号越小优先级越高，先对优先级高的节点进行匹配操作。不同型号的设备支持的取值范围不同，请以设备的实际情况为准。

【使用指导】

删除IPv6策略之前，必须先取消该IPv6策略在所有接口上的应用，否则删除失败。

配置**undo**命令时，如果指定了策略节点，将删除指定的节点；如果指定了节点模式，将按模式删除策略内所有与该模式匹配的所有节点；如果两者都没有指定，将删除整个策略。

【举例】

\# 配置一个IPv6策略aaa，其节点序列号为10，匹配模式为**permit**，并进入IPv6策略节点视图。
...
·**display ip****v6 policy-based-route**

**IPv6策略路由 \-- IPv6策略路由配置命令 \-- reset ipv6 policy-based-route statistics**

------------------------------------------------------------------------

**[reset ipv6 policy-based-route statistics**]命令用来清除IPv6策略路由的统计信息。
...
**[policy ***policy-name*]：清除指定IPv6策略的统计信息。*policy-name*表示策略名，唯一标识一个IPv6策略，为1～19个字符的字符串，区分大小写。

【使用指导】

系统按照策略名清除IPv6策略路由的统计信息。如果不指定策略名，将清除所有配置IPv6策略的匹配统计信息（该统计信息可以通过**display ipv6 policy-based-route interface**命令查看）；如果指定策略名，将清除指定IPv6策略的匹配统计信息。
...
·**display ip****v6 policy-based-route interface**

·**display ip****v6 policy-based-route local**
...
**[snmp-agent trap enable ipv6** **policy-based-route**]命令用来开启下一跳失效告警功能。

**[undo snmp-agent trap enable ipv6 policy-based-route**]命令用来关闭下一跳失效告警功能。
